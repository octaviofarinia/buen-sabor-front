...
export const Button = ({ callback, type, content, color, fullsize }: Props) => {
  const getContent = () => {
    switch (content) {
      case 'add':
        return <h5 className="lg:text-lg">Agregar registro</h5>;
      case 'x':
        return <FontAwesomeIcon icon={faXmark} size="lg" />;
      default:
        return <h5 className="lg:text-lg">{content}</h5>;
    }
  };
  const getColor = () => {
    switch (color) {
